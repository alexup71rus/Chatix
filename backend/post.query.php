...
switch ($_POST['request']) {
    ...
    case "SEND_PRIVATE_MESSAGE": // отправить личное сообщение пользователю
        ...
        if(!mysqli_num_rows($relations_result) && $_POST['id'] != 0) {
            ...
            $myTitle = $find_result['first_name']." ".$find_result['last_name'];
            $anyTitle = $info_result['first_name']." ".$info_result['last_name'];
            $isFound = false;
            if ($myTitle) {
        ...
        if($relations_result) {
            $unread_count = $relations_result['unread_count']+=1;
